...
public class ImageConverter {
    ...
    public static void main(String[] args) {
        ...
        //tests.ImageConverter.convertTiff(input);
    ...
    public static List<String> splitImage(String filePath, int quantidade) {
        ...
        String newPath = file.getParentFile().getAbsolutePath() + "\\splits" + "_" + nameFile;

        if (!new File(newPath).mkdirs()) {
            throw new RuntimeException("n�o consegui criar a pasta destino das imagens divididas. Talvez a pasta j� exista!");
        }

        double slices = Math.sqrt(quantidade);

        if (slices % 1 != 0) {
            throw new RuntimeException("numero deve ser quadrado perfeito");
        }
